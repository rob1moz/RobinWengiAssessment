...
class Xl_contract():
    ...
    def get_xs_loss_vector(self):
        primary_insurer_pattern = np.ones((1,len(self.inputs['claim_pattern']))) * self.inputs['claim_pattern']

        cumulative_individual_claim = self.monte_carlo.severity_vector[..., np.newaxis] * np.cumsum(primary_insurer_pattern)
...
class Portfolio(Xl_contract):
    def __init__(self, input_list):
        self.input_list = input_list

        # Define a dictionary with all the line of business objects
        self.lob = self.set_all_lob()


    def set_all_lob(self):
        lob ={}
        for i in self.input_list:
            lob[i['LOB']] = Xl_contract(i)
        return lob


    def get_portfolio_underwriting_risk(self, a_loss_kind = 'xs_loss'):
        premiums = []
        losses = []
